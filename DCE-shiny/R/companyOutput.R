...
buildCompanyOutput <- function(data_sheet) {
  ...
  data_sheet_scaled <- data_sheet %>% 
    #calculate total number of reported days by subtracting the period covered start date from the end date
  ...
  data_sheet_scaled_period_1 <- data_sheet_scaled
  data_sheet_scaled_period_2 <- data_sheet_scaled
  ...
  data_sheet_scaled_period_1$electricity_value_scaled_period <-
    ...
    ifelse(data_sheet_scaled_period_1$period_covered_start_year == 
             data_sheet_scaled_period_1$period_covered_end_year, data_sheet_scaled_period_1$electricity_value*1,
           data_sheet_scaled_period_1$electricity_value*data_sheet_scaled_period_1$period_start_proportion) 
  
  data_sheet_scaled_period_1$fuel_1_value_scaled_period <-
    
    #if start and end year are the same, multiply fuel 1 value by 1, else, multiply by the proportion of days in the first year
    ifelse(data_sheet_scaled_period_1$period_covered_start_year == 
             data_sheet_scaled_period_1$period_covered_end_year, data_sheet_scaled_period_1$fuel_1_value*1,
           data_sheet_scaled_period_1$fuel_1_value*data_sheet_scaled_period_1$period_start_proportion)
  
  data_sheet_scaled_period_1$fuel_2_value_scaled_period <-
    
    #if start and end year are the same, multiply fuel 2 value by 2, else, multiply by the proportion of days in the first year
    ifelse(data_sheet_scaled_period_1$period_covered_start_year == 
             data_sheet_scaled_period_1$period_covered_end_year, data_sheet_scaled_period_1$fuel_2_value*1,
           data_sheet_scaled_period_1$fuel_2_value*data_sheet_scaled_period_1$period_start_proportion)
  
  data_sheet_scaled_period_1$fuel_3_value_scaled_period <-
    
    #if start and end year are the same, multiply fuel 3 value by 3, else, multiply by the proportion of days in the first year
    ifelse(data_sheet_scaled_period_1$period_covered_start_year == 
             data_sheet_scaled_period_1$period_covered_end_year, data_sheet_scaled_period_1$fuel_3_value*1,
           data_sheet_scaled_period_1$fuel_3_value*data_sheet_scaled_period_1$period_start_proportion)
  
  data_sheet_scaled_period_1$fuel_4_value_scaled_period <-
    
    #if start and end year are the same, multiply fuel 4 value by 4, else, multiply by the proportion of days in the first year
    ifelse(data_sheet_scaled_period_1$period_covered_start_year == 
             data_sheet_scaled_period_1$period_covered_end_year, data_sheet_scaled_period_1$fuel_4_value*1,
           data_sheet_scaled_period_1$fuel_4_value*data_sheet_scaled_period_1$period_start_proportion)
  
  data_sheet_scaled_period_1$fuel_5_value_scaled_period <-
    
    #if start and end year are the same, multiply fuel 5 value by 5, else, multiply by the proportion of days in the first year
    ifelse(data_sheet_scaled_period_1$period_covered_start_year == 
             data_sheet_scaled_period_1$period_covered_end_year, data_sheet_scaled_period_1$fuel_5_value*1,
           data_sheet_scaled_period_1$fuel_5_value*data_sheet_scaled_period_1$period_start_proportion)
  
  data_sheet_scaled_period_1 <- data_sheet_scaled_period_1 %>% 
    ...
    #save only start date, end date, and data year (year data applies to)
  ...
  #created scaled values for period 2
  data_sheet_scaled_period_2$electricity_value_scaled_period <-
    
    #if start and end year are the same, multiply electricity value by 1, else, multiply by the proportion of days in the first year
    ifelse(data_sheet_scaled_period_2$period_covered_start_year == 
             data_sheet_scaled_period_2$period_covered_end_year, data_sheet_scaled_period_2$electricity_value*1,
           data_sheet_scaled_period_2$electricity_value*data_sheet_scaled_period_2$period_end_proportion) 
  
  data_sheet_scaled_period_2$fuel_1_value_scaled_period <-
    
    #if start and end year are the same, multiply fuel 1 value by 1, else, multiply by the proportion of days in the first year
    ifelse(data_sheet_scaled_period_2$period_covered_start_year == 
             data_sheet_scaled_period_2$period_covered_end_year, data_sheet_scaled_period_2$fuel_1_value*1,
           data_sheet_scaled_period_2$fuel_1_value*data_sheet_scaled_period_2$period_end_proportion)
  
  data_sheet_scaled_period_2$fuel_2_value_scaled_period <-
    
    #if start and end year are the same, multiply fuel 1 value by 1, else, multiply by the proportion of days in the first year
    ifelse(data_sheet_scaled_period_2$period_covered_start_year == 
             data_sheet_scaled_period_2$period_covered_end_year, data_sheet_scaled_period_2$fuel_2_value*1,
           data_sheet_scaled_period_2$fuel_2_value*data_sheet_scaled_period_2$period_end_proportion)
  
  data_sheet_scaled_period_2$fuel_3_value_scaled_period <-
    
    #if start and end year are the same, multiply fuel 1 value by 1, else, multiply by the proportion of days in the first year
    ifelse(data_sheet_scaled_period_2$period_covered_start_year == 
             data_sheet_scaled_period_2$period_covered_end_year, data_sheet_scaled_period_2$fuel_3_value*1,
           data_sheet_scaled_period_2$fuel_3_value*data_sheet_scaled_period_2$period_end_proportion)
  
  data_sheet_scaled_period_2$fuel_4_value_scaled_period <-
    
    #if start and end year are the same, multiply fuel 1 value by 1, else, multiply by the proportion of days in the first year
    ifelse(data_sheet_scaled_period_2$period_covered_start_year == 
             data_sheet_scaled_period_2$period_covered_end_year, data_sheet_scaled_period_2$fuel_4_value*1,
           data_sheet_scaled_period_2$fuel_4_value*data_sheet_scaled_period_2$period_end_proportion)
  
  data_sheet_scaled_period_2$fuel_5_value_scaled_period <-
    
    #if start and end year are the same, multiply fuel 1 value by 1, else, multiply by the proportion of days in the first year
    ifelse(data_sheet_scaled_period_2$period_covered_start_year == 
             data_sheet_scaled_period_2$period_covered_end_year, data_sheet_scaled_period_2$fuel_5_value*1,
           data_sheet_scaled_period_2$fuel_5_value*data_sheet_scaled_period_2$period_end_proportion)
  
  #drop all values where period covered start and end are equal prior to cbinding to the period 1 dataframe
  data_sheet_scaled_period_2 <- subset(data_sheet_scaled_period_2, period_covered_start_year != period_covered_end_year) 
  
  #relocate scaled column values to align with period 1 values
  data_sheet_scaled_period_2 <- data_sheet_scaled_period_2 %>% 
    relocate(electricity_value_scaled_period, .after = electricity_value) %>% 
    relocate(fuel_1_value_scaled_period, .after = fuel_1_value) %>% 
    relocate(fuel_2_value_scaled_period, .after = fuel_2_value) %>%
    relocate(fuel_3_value_scaled_period, .after = fuel_3_value) %>%
  ...
  data_sheet_scaled_combined <- rbind(data_sheet_scaled_period_1, data_sheet_scaled_period_2) %>%
  ...
  data_sheet <- data_sheet_scaled_combined %>% 
  ...
  #electricity and all fuel values on top of each other, drop columns with NA (filter by Spark for example plotting)
  ...
  #make data output reactive
  data_sheet_graph_1 <- reactive({
    data_sheet_graph_1 %>% 
      filter(company == "Spark")
  })
  
  # Stack bar graph output
  ggplot(data = data_sheet_graph_1(), aes(fill=fuel_type, y=value, x=data_year)) + 
