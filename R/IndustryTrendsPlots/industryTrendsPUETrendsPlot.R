buildIndustryTrendsPUETrends <- function(data_sheet_pue_filtered, selected_company, selected_scope) {
  data_sheet_pue_all <- data_sheet_pue_raw %>%
    select(c("company", "applicable_year", "geographical_scope", "pue_value", "pue_measurement_level")) %>%
    ...
    filter(!(company %in% selected_company) | !(geographical_scope_clean %in% selected_scope)) %>%
    na.omit()
  ...
  p <- ggplot(data=data_sheet_pue_all, aes(x=applicable_year, y=pue_value, 
    ...
    geom_point(data=data_sheet_pue_filtered, colour="#77945C", size=3) +
    theme_classic() +
    xlab("Year") +
    ylab("PUE Value") +
    labs(fill = "Companies")
