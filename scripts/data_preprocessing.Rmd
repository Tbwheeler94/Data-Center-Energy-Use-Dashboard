...
p <- ggplot(data_of_transparency, aes(x=data_year, text=paste("Data Year: ", data_year, "\nNumber of Companies: ", value))) + 
    geom_bar(aes(y=value, fill=energy_reporting_scope),
             position="stack", stat="identity") +
    ggtitle("Transparency Levels of Data Center Energy Use") +
    scale_fill_brewer() +
    theme_classic() +
    xlab("Year") +
...
#mutate(energy_reporting_scope = case_when("Single Data Center" ~ "Data Centers",
                                            #"Multiple Data Centers" ~ "Data Centers")) %>% 
for (year in 2007:2020) {
  ...
  energy_use_TO <- data_sheet_energy %>%
...
input_year <- 2020
input_reporting_scope <- "Data Centers"
energy_use_final <- energy_use_final %>%
  filter(data_year == input_year, energy_reporting_scope == input_reporting_scope)

if (input_reporting_scope == "Data Centers") {
  ...
  d <- 1
  e <- 1
  for (i in 1:max_company) {
    if (energy_use_L2[i,5] > 10000000000) {
         deleted_rows_1[a] <- i
         a <- a + 1
    } else if (energy_use_L2[i,5] > 1000000000 && energy_use_L2[i,5] < 10000000000) {
      deleted_rows_2[b] <- i
      b <- b + 1
    } else if (energy_use_L2[i,5] > 100000000 && energy_use_L2[i,5] < 1000000000) {
      deleted_rows_3[c] <- i
      c <- c + 1
    } else if (energy_use_L2[i,5] > 10000000 && energy_use_L2[i,5] < 100000000) {
      deleted_rows_4[d] <- i
      d <- d + 1
    } else {
      deleted_rows_5[e] <- i
  ...
  energy_use_L2_1 <- energy_use_L2[c(deleted_rows_1), ]
  energy_use_L2_2 <- energy_use_L2[c(deleted_rows_2), ]
  energy_use_L2_3 <- energy_use_L2[c(deleted_rows_3), ]
  energy_use_L2_4 <- energy_use_L2[c(deleted_rows_4), ]
  energy_use_L2_5 <- energy_use_L2[c(deleted_rows_5), ]
  ...
  p3e <- plot_ly() %>% 
    add_trace(x=energy_use_L2_5$electricity_value, 
              y=reorder(energy_use_L2_5$company, energy_use_L2_5$electricity_value),
