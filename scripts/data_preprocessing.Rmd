...
for (i in 1:nrow(data_of_transparency)) {
  ...
  } else if (data_of_transparency[i,2] == "" && data_of_transparency[i,3] == "Total Energy Use") {
...
p <- ggplot(data_of_transparency, aes(x=data_year)) + 
    ...
    geom_line(aes(y=number_of_companies), size=1.2, color="black") +
    ggtitle("Data Center Transparency") +
    scale_fill_manual(values = c("#3db5ff", "#0099f9", "#3d74ff", "#743dff")) +
    xlab("Year") +
    ylab("Percentage of Reporting Scope") +
    labs(fill = "Energy Reporting Scope")
...
filter(data_year == 2019, energy_reporting_scope == "Single Data Center") %>%
select(company, data_year, energy_reporting_scope, level_of_ownership, electricity_prepped) 

# create a sub data frame that is filtered by data year and multiple data centers scope
energy_use_MDC <- data_sheet_energy %>%
filter(data_year == 2019, energy_reporting_scope == "Multiple Data Centers") %>%
select(company, data_year, energy_reporting_scope, level_of_ownership, electricity_prepped)

# create a sub data frame that is filtered by data year and company wide electricity scope
energy_use_L2 <- data_sheet_energy %>%
filter(data_year == 2019, energy_reporting_scope == "Total Operations") %>%
select(company, data_year, energy_reporting_scope, level_of_ownership, electricity_prepped) 
...
# L2 CALCULATIONS
# erase companies from total operations if they report data center
energy_use_L1_subset <- distinct(energy_use_L1, company, .keep_all = TRUE)
duplicate_rows <- vector()
z = 1
for (i in 1:nrow(energy_use_L1_subset)) {
  for (j in 1:nrow(energy_use_L2)) {
    if (energy_use_L1_subset[i,1] == energy_use_L2[j,1]) {
      duplicate_rows[z] <- j
      z = z + 1
    }
  }
...
energy_use_L2 <- energy_use_L2[-c(duplicate_rows), ]

# subset L2 data
energy_use_L2 <- energy_use_L2 %>% 
...
cw <- ggplot(energy_use_L2, aes(x=company, y=unit, fill=electricity_value)) + 
    coord_flip() +
    scale_color_gradient2(low = "blue", mid = "snow", high = "red") +
    geom_bar(stat="identity") +
    ggtitle("Company Wide Electricity Use by Year") +
    theme_classic() +
    xlab("Electricity Value (KWh)") +
    ylab("Companies") +
    labs(fill = "Electricity Usage")

ggplotly(cw)

p1 <- plot_ly(data = energy_use_L2,
              x = ~electricity_value,
              y = ~company,
              color = ~company,
              colors = "Dark2",
              ...
              textposition = "auto",
              hoverinfo = "text",
              hovertext = paste("Electricity Value (KWh):", energy_use_L2$electricity_value))
p1

p2 <- plot_ly(data = energy_use_L2,
              x = ~unit,
              y = ~company,
              color = ~electricity_value,
              colors = "Dark2",
              type = "bar",
              text = ~company,
              textposition = "auto",
...
# 2nd Option
energy_use_L2 <- energy_use_L2[order(energy_use_L2[,"electricity_value"]), ]
max_company <- nrow(energy_use_L2)
loop_index <- nrow(energy_use_L2)-1
...
for (i in 1:loop_index) {
  if (energy_use_L2[i,5] > energy_use_L2[max_company,5]/1000) {
    deleted_rows_1[a] <- i
    a <- a + 1
  } else if (energy_use_L2[i,5] > energy_use_L2[max_company,5]/1000000) {
    deleted_rows_2[b] <- i
    b <- b + 1
  } else {
    deleted_rows_3[c] <- i
...
energy_use_L2_1 <- energy_use_L2[c(max_company), ]
energy_use_L2_2 <- energy_use_L2[c(deleted_rows_1), ]
energy_use_L2_3 <- energy_use_L2[c(deleted_rows_2), ]
energy_use_L2_4 <- energy_use_L2[c(deleted_rows_3), ]

p3a <- plot_ly() %>% 
  add_trace(x=energy_use_L2_1$electricity_value, 
            y=reorder(energy_use_L2_1$company, energy_use_L2_1$electricity_value), 
            type="bar", marker = list(color = "rgb(232, 218, 239)"))

p3b <- plot_ly() %>% 
  add_trace(x=energy_use_L2_2$electricity_value, 
            y=reorder(energy_use_L2_2$company, energy_use_L2_2$electricity_value),
            type="bar", marker = list(color = "rgb(210, 180, 222)"))

p3c <- plot_ly() %>% 
  add_trace(x=energy_use_L2_3$electricity_value, 
            y=reorder(energy_use_L2_3$company, energy_use_L2_3$electricity_value),
            type="bar", marker = list(color = "rgb(187, 143, 206)"))

p3d <- plot_ly() %>% 
  add_trace(x=energy_use_L2_4$electricity_value, 
            y=reorder(energy_use_L2_4$company, energy_use_L2_4$electricity_value),
            type="bar", marker = list(color = "rgb(165, 105, 189)"))

p3 <- subplot(p3a,p3b,p3c,p3d,nrows = 4, margin = 0.05)
p3
...
#####################################################################
################# Export data sheet #################################
# Contents: Data sheet energy after transformation (up to line 462) #
#####################################################################

write.csv(data_sheet_energy, here("data", "data_sheet_energy_transformed.csv"))
...
graph_by_industry_total <- data_sheet_energy %>% 
  ...
  select("company", "data_year", "energy_reporting_scope", "level_of_ownership", "electricity_prepped", "unit", "notes_2") %>% 
