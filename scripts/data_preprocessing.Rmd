...
## Data import and prepping spreadsheet for calculating standardized energy values (server.R)
...
colnames(data_sheet) [6] <- 'level_of_ownership'
colnames(data_sheet) [9] <- 'electricity_value'
colnames(data_sheet) [10] <- 'unit_scale'
colnames(data_sheet) [15] <- 'fuel_1_value'
colnames(data_sheet) [16] <- 'fuel_1_unit_scale'
colnames(data_sheet) [20] <- 'fuel_2_value'
colnames(data_sheet) [21] <- 'fuel_2_unit_scale'
colnames(data_sheet) [25] <- 'fuel_3_value'
colnames(data_sheet) [26] <- 'fuel_3_unit_scale'
colnames(data_sheet) [30] <- 'fuel_4_value'
colnames(data_sheet) [31] <- 'fuel_4_unit_scale'
colnames(data_sheet) [35] <- 'fuel_5_value'
colnames(data_sheet) [36] <- 'fuel_5_unit_scale'

```

## Collect and transform reported energy values that are split across years
...
graph_by_fuel_type <- 
  rbind(data_sheet_electricity, data_sheet_combined_1, #stack converted electricity, fuel values sheets on top of each other
...
graph_by_data_center <- data_sheet %>% 
...
graph_by_industry_total <- data_sheet %>% 
  mutate_at(vars(electricity_converted, fuel_1_converted, #replace na values with 0
           fuel_2_converted, fuel_3_converted, fuel_4_converted, 
           fuel_5_converted), ~replace_na(., 0)) %>%
  rowwise() %>% 
...
write.csv(graph_by_industry_total, "aggregate_data.csv")
write.csv(graph_by_fuel_type, "by_fuel_type_data.csv")

graph_by_industry_total_company <- graph_by_industry_total %>% 
  filter(grepl("Data Centers", company))

graph_by_industry_datacenters <- graph_by_industry_total %>% 
  filter(grepl("Total Company Energy Use", company))

paste(round(sum(graph_by_industry_total$value)/1000000000, 1), "TWh")
```

```{r}
# Stacked

graph_by_fuel_type <- by_fuel_type_data %>% 
    filter(company == "Spark")
           
ggplot(graph_by_fuel_type, aes(fill=fuel_type, y=value, x=data_year)) + 
    geom_bar(position="stack", stat="identity")

```

```{r}
ggplot(graph_by_data_center, aes(fill=scope_display_label, y=total_energy_use, x=data_year)) + 
    geom_bar(position="stack", stat="identity")
```
