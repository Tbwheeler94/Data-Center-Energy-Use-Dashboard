...
data_sheet_energy <- read.xlsx2(here('data',"DataCenterEnergyUse-RawCollection.xlsx"), 1, #the "1" specifies you want sheet 1
...
data_sheet_company <- read.xlsx2(here('data',"DataCenterEnergyUse-RawCollection.xlsx"), 2, #the "2" specifies you want sheet 2
                         
                             #specify column data types to ensure proper recognition
                             colClasses=c("character","character","character","Date","character", #columns 1-5
...
#change column names that were set to incorrect values when column classes were set
colnames(data_sheet_company) [3] <- 'date_last_updated'

```

## Collect and transform reported energy values that are split across 2 years
...
filter(data_year == 2007, energy_reporting_scope == "Single Data Center") %>%
...
filter(data_year == 2007, energy_reporting_scope == "Multiple Data Centers") %>%
select(company, data_year, energy_reporting_scope)

data_of_transparency_MDC <- distinct(data_of_transparency_MDC, .keep_all = TRUE)

data_of_transparency_TO <- data_sheet_energy %>%
filter(data_year == 2007, energy_reporting_scope == "Total Operations") %>%
select(company, data_year, energy_reporting_scope) 

data_of_transparency_TO <- distinct(data_of_transparency_TO, .keep_all = TRUE)

data_of_transparency_ND <- data_sheet_energy %>%
filter(data_year == 2007, energy_reporting_scope == "") %>%
select(company, data_year, energy_reporting_scope)

data_of_transparency_ND <- distinct(data_of_transparency_ND, .keep_all = TRUE)

#stack single data center/multiple data center/TO sheets on top of each other
data_of_transparency <- rbind(data_of_transparency_SDC, data_of_transparency_MDC, 
                              data_of_transparency_TO, data_of_transparency_ND)
...
data_of_transparency$value <- nrow(data_of_transparency)

for (year_count in 2008:2020) {
  data_of_transparency_SDC <- data_sheet_energy %>%
  ...
  #stack single data center/multiple data center sheets on top of each other
...
data_founding_year <- data_founding_year[!apply(data_founding_year == "", 1, all),]
deleted_rows <- vector()
z = 1
...
for (i in 1:nrow(data_of_transparency)) {
  if (data_of_transparency[i,2] == "Single Data Center" || data_of_transparency[i,2] == "Multiple Data Centers") {
    data_of_transparency[i,2] = "Reported Data Center Energy Use"
  } else if (data_of_transparency[i,2] == "Total Operations") {
    data_of_transparency[i,2] = "Reported Company Wide Energy Use"
  } else if (data_of_transparency[i,2] == "") {
    data_of_transparency[i,2] = "Issue Report With No Data"
  } else {
    data_of_transparency[i,2] = "Issue No Report"
  }
...
p <- ggplot(data_of_transparency, aes(x=data_year, y=unit)) + 
    geom_bar(aes(x=data_year, y=value, fill=energy_reporting_scope), 
             position=position_fill(reverse = TRUE), stat="identity") +
    #geom_line(aes(x=data_year, y=value), size=1.2, color="black") +
